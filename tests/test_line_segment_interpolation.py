...
class LineSegmentInterpolationTest(unittest.TestCase):
    ...
    def test_line_right_up(self):
        line_seg = self.create_line_segment((1, 1), (3, 3))
        self.assertEquals(interpolate_within_line_segment(line_seg, 2.5), 2.5)
        self.assertEquals(interpolate_within_line_segment(line_seg, 2), 2.0)
        
    def test_line_right_down(self):
        line_seg = self.create_line_segment((1, 1), (3, -1))
        self.assertEquals(interpolate_within_line_segment(line_seg, -0.5), 2.5)
        self.assertEquals(interpolate_within_line_segment(line_seg, 2), 0.0)
        
    def test_line_left_up(self):
        line_seg = self.create_line_segment((2, 2), (0, 4))
        self.assertEquals(interpolate_within_line_segment(line_seg, 0.5), 3.5)
        self.assertEquals(interpolate_within_line_segment(line_seg, 1), 3)
        
    def test_line_left_down(self):
        line_seg = self.create_line_segment((2, 2), (0, 0))
        self.assertEquals(interpolate_within_line_segment(line_seg, 0.5), 0.5)
        self.assertEquals(interpolate_within_line_segment(line_seg, 1), 1)

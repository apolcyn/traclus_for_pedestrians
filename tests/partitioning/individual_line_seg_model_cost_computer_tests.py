...
from partitioning.trajectory_partitioning import individial_line_seg_model_cost_computer
import math
...
class IndividualLineSegModelCostComputerTest(UnitBaseTests):
    ...
    def test_get_cost_normal_case(self):
        ...
        self.assertEquals(individial_line_seg_model_cost_computer(line_seg), 1)
        
        line_seg = self.create_simple_line_seg(start=(0, 0), end=(4, 4))
        self.assertEquals(individial_line_seg_model_cost_computer(line_seg), 3)

        line_seg = self.create_simple_line_seg(start=(1, 0), end=(1 + math.sqrt(3) + 0.1, 1))
        self.assertEquals(individial_line_seg_model_cost_computer(line_seg), 2)
        self.assertEquals(individial_line_seg_model_cost_computer(line_seg), 2)
        ...
        self.assertEquals(individial_line_seg_model_cost_computer(line_seg), 0) 
        
    def test_get_cost_bad_inputs(self):
        line_seg = self.create_simple_line_seg(start=(2, 3), end=(2, 3.99999))
        self.assertRaises(ValueError, individial_line_seg_model_cost_computer, line_seg) 
